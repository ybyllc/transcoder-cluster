...
### v0.2.1 (当前版本)
- ✅ 修复主控端任务列表进度卡在“上传中 0%”的问题，上传阶段可显示实时百分比
- ✅ 修复节点状态偶发 `unknown` 覆盖“处理中”的问题，状态回退逻辑更稳健
- ✅ Worker 服务改为并发 HTTP 处理，`/task` 执行期间 `/status` 仍可实时响应
- ✅ CRF/CQ 支持输入 `0` 表示自动（不追加 `-crf/-cq`），提示文案同步优化
- ✅ 左侧流程栏支持最小/最大宽度约束，默认宽度与拖拽行为更稳定
- ✅ 转码配置与输出后缀提示统一改为悬浮提示，界面信息更简洁
- ✅ 新增通信流程技术笔记：`transcoder_cluster/core/COMMUNICATION_FLOW.md`
- ✅ Release 工作流支持版本专属详细更新日志，发布页不再依赖 README 链接
