...
示例:
  ...
  tc-worker --config worker.json
    ...
    parser.add_argument("--port", "-p", type=int, default=9000, help="Worker 监听端口 (默认: 9000)")
    parser.add_argument(
        "--work-dir",
        "-w",
        type=str,
    ...
    parser.add_argument("--no-discovery", action="store_true", help="禁用节点发现服务")
    parser.add_argument(
        "--log-level",
        type=str,
        default="INFO",
    ...
    if not args.no_discovery:
        # 心跳服务
        heartbeat = HeartbeatService(get_status=lambda: Worker.get_status())

        # 发现响应器
        responder = DiscoveryResponder(get_status=lambda: Worker.get_status())
